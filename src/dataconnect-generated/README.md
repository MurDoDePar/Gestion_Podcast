...
// Variables can be defined inline as well.
const { data } = await getRecommendations({ feedUrl: ..., });
...
// Variables can be defined inline as well.
const ref = getRecommendationsRef({ feedUrl: ..., });
...
You can execute the `GetEpisodesByPodcast` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getEpisodesByPodcast(vars: GetEpisodesByPodcastVariables, options?: ExecuteQueryOptions): QueryPromise<GetEpisodesByPodcastData, GetEpisodesByPodcastVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
getEpisodesByPodcast(dc: DataConnect, vars: GetEpisodesByPodcastVariables, options?: ExecuteQueryOptions): QueryPromise<GetEpisodesByPodcastData, GetEpisodesByPodcastVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getEpisodesByPodcastRef:
```typescript
const name = getEpisodesByPodcastRef.operationName;
console.log(name);
```

### Variables
The `GetEpisodesByPodcast` query requires an argument of type `GetEpisodesByPodcastVariables`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
...
```
### Return Type
Recall that executing the `GetEpisodesByPodcast` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetEpisodesByPodcastData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetEpisodesByPodcast`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getEpisodesByPodcast, GetEpisodesByPodcastVariables } from '@dataconnect/generated';

// The `GetEpisodesByPodcast` query requires an argument of type `GetEpisodesByPodcastVariables`:
const getEpisodesByPodcastVars: GetEpisodesByPodcastVariables = {
  podcastId: ..., 
};

// Call the `getEpisodesByPodcast()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getEpisodesByPodcast(getEpisodesByPodcastVars);
// Variables can be defined inline as well.
...
// Or, you can use the `Promise` API.
getEpisodesByPodcast(getEpisodesByPodcastVars).then((response) => {
  const data = response.data;
  console.log(data.episodes);
});
```

### Using `GetEpisodesByPodcast`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getEpisodesByPodcastRef, GetEpisodesByPodcastVariables } from '@dataconnect/generated';
...
// Call the `getEpisodesByPodcastRef()` function to get a reference to the query.
const ref = getEpisodesByPodcastRef(getEpisodesByPodcastVars);
// Variables can be defined inline as well.
const ref = getEpisodesByPodcastRef({ podcastId: ..., });

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getEpisodesByPodcastRef(dataConnect, getEpisodesByPodcastVars);
...
You can execute the `GetLatestSubscribedEpisodes` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getLatestSubscribedEpisodes(vars: GetLatestSubscribedEpisodesVariables, options?: ExecuteQueryOptions): QueryPromise<GetLatestSubscribedEpisodesData, GetLatestSubscribedEpisodesVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
getLatestSubscribedEpisodes(dc: DataConnect, vars: GetLatestSubscribedEpisodesVariables, options?: ExecuteQueryOptions): QueryPromise<GetLatestSubscribedEpisodesData, GetLatestSubscribedEpisodesVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getLatestSubscribedEpisodesRef:
```typescript
const name = getLatestSubscribedEpisodesRef.operationName;
console.log(name);
```

### Variables
The `GetLatestSubscribedEpisodes` query requires an argument of type `GetLatestSubscribedEpisodesVariables`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
...
```
### Return Type
Recall that executing the `GetLatestSubscribedEpisodes` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetLatestSubscribedEpisodesData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetLatestSubscribedEpisodes`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getLatestSubscribedEpisodes, GetLatestSubscribedEpisodesVariables } from '@dataconnect/generated';

// The `GetLatestSubscribedEpisodes` query requires an argument of type `GetLatestSubscribedEpisodesVariables`:
const getLatestSubscribedEpisodesVars: GetLatestSubscribedEpisodesVariables = {
  userId: ..., 
};

// Call the `getLatestSubscribedEpisodes()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getLatestSubscribedEpisodes(getLatestSubscribedEpisodesVars);
// Variables can be defined inline as well.
const { data } = await getLatestSubscribedEpisodes({ userId: ..., });
...
// Or, you can use the `Promise` API.
getLatestSubscribedEpisodes(getLatestSubscribedEpisodesVars).then((response) => {
  const data = response.data;
  console.log(data.subscriptionTypes);
});
```

### Using `GetLatestSubscribedEpisodes`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getLatestSubscribedEpisodesRef, GetLatestSubscribedEpisodesVariables } from '@dataconnect/generated';
...
// Call the `getLatestSubscribedEpisodesRef()` function to get a reference to the query.
const ref = getLatestSubscribedEpisodesRef(getLatestSubscribedEpisodesVars);
// Variables can be defined inline as well.
const ref = getLatestSubscribedEpisodesRef({ userId: ..., });
