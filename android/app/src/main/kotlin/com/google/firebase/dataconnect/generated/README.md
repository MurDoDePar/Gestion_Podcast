...
with service ID `podstream-a980a-service`
hosted in `europe-west9`.
...
is the query named "FindUserByGoogleId",
which can be accessed via the [ExampleConnector.findUserByGoogleId] property.
...
For example, the "FindUserByGoogleId" query has 1 required variable ("googleId")
and can be executed via the [ExampleConnector.findUserByGoogleId]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.findUserByGoogleId.execute(googleId="waldo")
println("FindUserByGoogleId query returned: ${queryResult.data}")
```
...
For example, the "UpdateListenHistory" mutation has 5 required variables ("userId", "episodeId", "progressSeconds", "finishedListening", and "listenedAt")
and can be executed via the [ExampleConnector.updateListenHistory]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.updateListenHistory.execute(userId=java.util.UUID.randomUUID(), episodeId=java.util.UUID.randomUUID(), progressSeconds=3511784, finishedListening=false, listenedAt=Timestamp(1729400766, 257175589))
println("UpdateListenHistory mutation returned: ${mutationResult.data}")
```
...
For example, the "UpsertPodcast" mutation has 5 optional variables ("id", "description", "imageUrl", "author", and "categories")
and can be executed via the [ExampleConnector.upsertPodcast]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.upsertPodcast.execute(title="thud", feedUrl="waldo", createdAt=Timestamp(529506644, 777595443)) {
...
println("UpsertPodcast mutation returned: ${mutationResult.data}")
```
